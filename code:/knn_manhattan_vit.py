...
# ---- Load saved ViT-B/16 features ----
data = torch.load("train_features_vit_b16.pt", weights_only=False)
...
# ---- Load ViT-B/16 for encoding test images ----
...
# ============================================================
# PART 1: 5-Fold Cross-Validation (Manhattan / L1 distance)
# ============================================================
print(f"KNN Classifier: k={K}, distance=Manhattan (L1)")
print(f"Encoder: ViT-B/16, Features: {train_features.shape}")
print(f"{'='*60}")
...
for fold, (tr_idx, va_idx) in enumerate(skf.split(train_features_norm, train_labels), 1):
    ...
    dists = np.abs(va_X[:, None] - tr_X[None, :]).sum(axis=2)
...
print(f"\n{'='*60}")
print(f"Encoding 10 test images with ViT-B/16...")
test_features = []
with torch.no_grad():
...
# Manhattan KNN predict
dists = np.abs(test_features_norm[:, None] - train_features_norm[None, :]).sum(axis=2)
topk_idx = np.argpartition(dists, K, axis=1)[:, :K]
topk_dist = np.take_along_axis(dists, topk_idx, axis=1)
sort_order = np.argsort(topk_dist, axis=1)
topk_idx_sorted = np.take_along_axis(topk_idx, sort_order, axis=1)
...
print(f"\n{'='*60}")
print(f"KNN Predictions on 10 Unseen Test Images (k={K}, Manhattan)")
print(f"{'='*60}")
